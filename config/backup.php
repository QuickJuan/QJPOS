    ...
    'backup' => [
        ...
        'name' => function() {
            if (app()->bound('tenant') && tenant()) {
                return env('BACKUP_NAME', env('APP_NAME', 'QuickJuan-POS')) . '-tenant-' . tenant('id');
            }
            return env('BACKUP_NAME', env('APP_NAME', 'QuickJuan-POS')) . '-central';
        },
        ...
        'source' => [
            ...
            'databases' => function() {
                if (app()->bound('tenant') && tenant()) {
                    // For tenant context, backup only the tenant database
                    return ['tenant'];
                }
                // For central context, backup the central database
        ...
        'destination' => [
            ...
            'filename_prefix' => function() {
                if (app()->bound('tenant') && tenant()) {
                    // For tenant context, prefix with tenant database name
                    return tenant('id') . '_' . config('database.connections.tenant.database', 'tenant') . '_';
                }
                // For central context, prefix with central identifier
                return 'central_' . config('database.connections.' . config('database.default') . '.database', 'central') . '_';
            },
            ...
            'disks' => function() {
                if (app()->bound('tenant') && tenant()) {
                    // For tenant backups, store in tenant-specific directory
                    return ['tenant-backups'];
                }
                // For central backups, store in central directory
                return ['central-backups'];
            },
        ],
